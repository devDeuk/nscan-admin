...
// This is the ols search Filter method used in navigator.
			...
			if($.fn.searchFilter) {
				...
				if( $("#"+fid).html() !== null ) {
					if ( $.isFunction(p.beforeShowSearch) ) {
						show${resources}/img = p.beforeShowSearch($("#"+fid));
						if(typeof(show${resources}/img) == "undefined") {
							show${resources}/img = true;
						}
					}
	...
	filterGrid : function(gridid,p){
		...
		return this.each(function(){
			...
			var ${resources}/img = $("<form name='SearchForm' style=display:inline;' class='"+this.p.formclass+"'></form>");
			tbl =$("<table class='"+this.p.tableclass+"' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");
			$(${resources}/img).append(tbl);
			formFill();
			$(this).append(${resources}/img);
