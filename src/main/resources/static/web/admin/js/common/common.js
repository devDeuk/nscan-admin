...
var QUART_APLY_OBJ_OP_CD = 'QUART_APLY_OBJ_OP_CD';
...
var popupData = { cd:'', nm:'', sale_chnl:'', chnl_cd:'', org_id:'', eqp_mdl_cd:'', scrb_cl_cd:'', scrb_cl_nm:'', spmall_url:'', spmall_cl:'', spmall_abbr:''
	...
	// 2019 단말기 색상
	,eqp_color_cd:'',eqp_color_nm:'',eqp_2g3g_cl_nm:'',prod_co_nm:''
};
...
function resetPopupData(key) {
	switch(key) {
		case 'eqpSearch' :
			...
			popupData.eqp_2g3g_cl_nm = '';
			popupData.eqp_color_cd = '';
			popupData.eqp_color_nm = '';
			popupData.prod_co_nm = '';
			break;
...
var eqpSearch       = 'javascript:windowOpen(\"/common/eqpSrchList.do?popType=single\", \"EqpSearch\", \"770\", \"650\", \"no\", \"eqpSearch\");';					// 단말기 찾기 팝업
var eqpSearchF      = 'javascript:windowOpen(\"/common/eqpSrchList.do?popType=multi\", \"EqpSearch\", \"770\", \"750\", \"no\", \"eqpSearch\");';					// 단말기 찾기 팝업
...
function createGridButtons(img_path) {
	...
	toolbarMateAdd  = '<button type="button" class="btn btn-gray" style="cursor:pointer;vertical-align:middle;" id="addMateProdBtn" data-mateyn="Y"><img src="'+img_path+'/common/bul/add.png"  title="T렌탈상품추가"     /> T렌탈상품추가</button>';
...
/*
 * TWF 매장별 쿼터 관리시 매장 조회를 위해서 추가생성
 */
function commOrgShopList(fObj, sale) {

	console.log("commOrgShopList : ", sale );
	...
	$(selObj).change( function() {
		...
		$('#frm').find('input[name=mktg_org_lvl_cd]').val((id == '20') ? '30' : (id == '30') ? '40' : 'S');
			...
			, success  : function(data, stauts, request) {
				...
				for(var i = 0; i < obj.length; i++) {
					...
					if(obj[i] != undefined) {
						for(var j = 0; j < obj[i].length; j++) {
							if(id == '40' && j > 0) {
								option += '<option value="'+((obj[i][j].ORG_ID == null) ? 'NULL' : obj[i][j].ORG_ID)+'">['+obj[i][j].SALE_BR_ORG_CD+'] '+obj[i][j].SALE_BR_ORG_NM+'</option>';
							} else {
								option += '<option value="'+((obj[i][j].ORG_ID == null) ? 'NULL' : obj[i][j].ORG_ID)+'">'+obj[i][j].ORG_NM+'</option>';
							}
						}
...
/*
 * TWF 통계 매장 조회를 위해서 추가생성
 */
function commOrgTwfShopList(fObj, sale) {

	console.log("commOrgTwfShopList : ", sale );
	...
	$(selObj).change( function() {
		...
		$('#frm').find('input[name=mktg_org_lvl_cd]').val((id == '20') ? '30' : (id == '30') ? '40' : 'W');
			...
			, success  : function(data, stauts, request) {
				...
				for(var i = 0; i < obj.length; i++) {
					...
					if(obj[i] != undefined) {
						for(var j = 0; j < obj[i].length; j++) {
							if(id == '40' && j > 0) {
								option += '<option value="'+((obj[i][j].SALE_BR_ORG_CD == null) ? 'NULL' : obj[i][j].SALE_BR_ORG_CD)+'">['+obj[i][j].SALE_BR_ORG_CD+'] '+obj[i][j].SALE_BR_ORG_NM+'</option>';
...
function commOrgList(fObj, sale) {
	...
	$(selObj).change( function() {
		...
		$('#frm').find('input[name=mktg_org_lvl_cd]').val((id == '20') ? '30' : (id == '30') ? '40' : 'P');
		...
		, success  : function(data, stauts, request) {
			...
			if(obj != undefined) {
				for(var i = 0; i < obj.length; i++) {
					...
					for(var j = 0; j < obj[i].length; j++) {
						if(i == 3) {
							option += '<option value="'+obj[i][j].USER_ID+'" sale_chnl="'+obj[i][j].SALE_CHNL+'" tfriends_yn="'+obj[i][j].TFRIENDS_YN+'">'+obj[i][j].ORG_NM+'</option>'; // 판매자일 경우 가져오는 DB값이 다름, sale_chnl 필수(중요:*****)
						} else {
				...
				if(obj != undefined) {
					for(var i = 0; i < obj.length; i++) {
						if(mktg_org_lvl_cd == '40') {
							option += '<option value="'+obj[i].USER_ID+'" sale_chnl="'+obj[i].SALE_CHNL+'" tfriends_yn="'+obj[i].TFRIENDS_YN+'" >'+obj[i].ORG_NM+'</option>'; // 판매자일 경우 가져오는 DB값이 다름, sale_chnl 필수(중요:*****)
						} else {
						...
						.on('keydown', function(e){
							...
							if( code == 13 )
							{
								...
								if(currText != $inputTab.val()) {
									...
									$.ajax({
										url      : '/prod/wlessProdTabUpt.do?' + $('#frm').serialize()
										, type     : 'POST'
										, dataType : 'JSON'
										, cache    : false
										, success  : function(data, stauts, request) {
...
function atchFileImgView(fullpathname, encYn) {
	//document.getElementById(div).style.height=height;
	//alert(fullpathname);
	//window.open('/tgate/common/imgview.jsp?spath='+fullpathname);
	var win = window.open("/tgate/common/imgview.jsp?spath="+ fullpathname +'&enc=' + encYn , "imgview", "titlebar=no, location=no, statusbar=no, toolbar=no, scrollbars=yes, menubar=no, resizable=yes, width=800, height=600");
	win.focus();
}
